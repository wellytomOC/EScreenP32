...
void WifiMain(void *parameter)
{
    ...
    ArduinoOTA.setHostname("DisplayWash");
...
void OtaMain(void *parameter)
{
    ...
    setupOTA("DisplayWash",WIFI_SSID,WIFI_PWD);
    while (1)
    {
        Serial.println("OTA");
        ArduinoOTA.handle();
...
void InitWifi(void)
{
    ...
    xTaskCreate(OtaMain, "OtaMain", 10000, NULL, 1, HandlerOTA);
}
